...
namespace Ultrapain.Patches
{
    ...
    {
        static bool Prefix(Sisyphus __instance, EnemyIdentifier ___eid)
        {
            Vector3 vector = __instance.transform.position + Vector3.up;
            if (Physics.Raycast(vector, PlayerTracker.instance.GetTarget().position - vector, Vector3.Distance(PlayerTracker.instance.GetTarget().position, vector), LayerMaskDefaults.Get(LMD.Environment)))
            {
