    ...
    class Obamapticon_Start
    {
        ...
        static void Postfix(FleshPrison __instance)
        {
            ...
            if (__instance.bossHealth != null)
            {
                ...
                if (__instance.bossHealth.bossName != null)
                {
                    BossHealthBarTemplate temp = __instance.bossHealth.GetComponent<BossHealthBarTemplate>();
                    temp.bossNameText.text = ConfigManager.obamapticonName.value;
                    foreach (TMPro.TMP_Text t in temp.textInstances)
    ...
    class Panopticon_SpawnInsignia
    {
            ...
            ref float ___fleshDroneCooldown, EnemyIdentifier ___eid)
        {
            ...
            if (gameObject.TryGetComponent<VirtueInsignia>(out virtueInsignia))
            {
                ...
                virtueInsignia.explosionLength = 3f;
                ...
                virtueInsignia.target = new EnemyTarget(MonoSingleton<PlayerTracker>.Instance.GetPlayer());
                virtueInsignia.target.isPlayer = true;
            ...
            GameObject createInsignia(EnemyIdentifier eid, int difficulty, EnemyTarget target, int damage = 30, float lastMultiplier = 1)
            {
                GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(Plugin.virtueInsignia, target.position, Quaternion.identity);
            ...
            float lastingmulti = 1f;
            float size = ConfigManager.panopticonAxisBeamSizeMulti.value * 12f;
            EnemyTarget et = ___eid.target;
            GameObject objx = createInsignia(___eid, ___difficulty, et, 30, lastingmulti);
            objx.transform.localScale = new Vector3(size, objx.transform.localScale.y, size);
            objx.transform.Rotate(new Vector3(90f, 0, 0));

            GameObject objy = createInsignia(___eid, ___difficulty, et, 30, lastingmulti);
            objy.transform.localScale = new Vector3(size, objy.transform.localScale.y, size);

            GameObject objz = createInsignia(___eid, ___difficulty, et, 30, lastingmulti);
            objz.transform.localScale = new Vector3(size, objz.transform.localScale.y, size);
            objz.transform.Rotate(new Vector3(0, 0, 90f));
            if (___fleshDroneCooldown < 1f)
            {
                ___fleshDroneCooldown = 1f;
            }
    ...
    class Panopticon_BlueProjectile
    {
        public static void BlueProjectileSpawn(FleshPrison instance)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                comp.target = new EnemyTarget(MonoSingleton<PlayerTracker>.Instance.GetTarget());
